...
def GetNessusVulDetails(TaskID,Application,single_nessus_xml):
	...
	hosts_xml=xml["NessusClientData_v2"]["Report"]["ReportHost"]
	
	pool=Pool(size=8)
	threads=[pool.spawn(process_host,host) for host in hosts_xml]
	pool.join()
